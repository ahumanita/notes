...
\begin{Assumption}{mapping-decomposition}
  ...
  or volume. Furthermore, we assume $\abs{\det \Phi}>0$.
\end{Assumption}

\begin{figure}
  \centering
  \includegraphics[width=.6\textwidth]{graph/mapping-decomposition.tikz}
  \caption{Visualization of $\Phi^{-1}$ which maps a quadrilateral mesh cell $\cell$ to the reference cell $\refcell$.}
\end{figure}
